...
class AppState:
    ...
    def threat_level(self, value: int):
        ...
        # append and maintain deque
        self.threat_history.append(level)
        # Broadcast state change via virtual event
    ...
    def reset_history(self):
        """Clears the threat history and resets the threat level visually."""
        self.threat_history.clear()
        self.threat_history.extend([0] * MAX_THREAT_HISTORY)
        self.threat_level = 0  # this triggers event


# --- Utility: Simple cross-platform Sound Manager ---
class SoundManager:
...
# --- DPI awareness helper (Windows friendly, safe on other platforms) ---
def enable_dpi_awareness(root):
    """
    Attempt to enable DPI awareness on Windows so scaling/pixels are correct.
    Also sets a tk scaling based on the system DPI so fonts and coordinates scale.
    ...
    try:
        ...
        # Force an update so winfo_fpixels('1i') returns a correct DPI value
        root.update_idletasks()
        # pixels per inch for the display (72 is Tk default logical DPI)
        pixels_per_inch = root.winfo_fpixels('1i')
        # compute a scaling factor relative to Tk's default 72 DPI
        scaling = max(0.6, min(2.5, pixels_per_inch / 72.0))
        # apply the scaling to Tk so widget sizes & fonts scale
        try:
            root.tk.call('tk', 'scaling', scaling)
        except Exception:
            pass

        # Optionally adjust small global font sizes (Consolas used in many places).
        base_font_size = max(8, int(10 * scaling))
        root.option_add("*Font", f"Consolas {base_font_size}")
    except Exception:
        pass
...
class AnimatedBackground(tk.Canvas):
    ...
    def __init__(self, parent, particle_count=DEFAULT_PARTICLE_COUNT, fps=30, **kwargs):
        ...
        width = self.winfo_reqwidth() or self.winfo_width() or 1200
        height = self.winfo_reqheight() or self.winfo_height() or 800
    ...
    def draw_grid(self):
        ...
        step = max(40, int(min(width, height) / 15))
...
class HologramGlobe(tk.Canvas):
    def __init__(self, parent, state: AppState, radius=110, points=260, fps=35, **kwargs):
        ...
        # ensure center uses actual size after layout
        self.after(50, self.on_resize)
    ...
    def draw_connections(self, cx, cy, ratio, glitch_intensity):
        ...
        connections_drawn = 0
        max_draw = int(self.max_connections * (ratio * 0.8 + 0.2))
...
class CyberpunkThreatMatrix(tk.Tk):
    def __init__(self):
        ...
        # Use a responsive default geometry based on screen size (instead of fixed)
        self.update_idletasks()
        sw = self.winfo_screenwidth()
        sh = self.winfo_screenheight()
        default_w = min(1600, int(sw * 0.85))
        default_h = min(1000, int(sh * 0.85))
    ...
    def update_threat_meter_visuals(self):
        ...
        for i in range(3, 0, -1):
            ...
            r, g, b = [c + (255 - c) * 0.1 * i for c in base_color_rgb]
