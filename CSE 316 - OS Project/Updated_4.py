...
class HologramGlobe(tk.Canvas):
    """3D Globe integrated into the Advanced UI"""
    ...
    def animate(self):
        ...
        # Outer Radar Ring
        self.create_oval(cx-r-10, cy-r-10, cx+r+10, cy+r+10, outline=C_GRID, width=1)
        if self.state.is_running:
            angle = (time.time() * 200) % 360
            rad = math.radians(angle)
            lx = cx + math.cos(rad) * (r+15)
            ly = cy + math.sin(rad) * (r+15)
            self.create_line(cx, cy, lx, ly, fill=C_ACCENT_2 if self.state.threat_level > 50 else C_ACCENT_4, stipple='gray50')
